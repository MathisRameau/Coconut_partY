...
void ask_monkey_position(Case** game_board, Case* chemin, int chemin_length, int height, int width, int* x, int* y) {
    ...
    int x_choisie, y_choisie;
    typedef struct {
        int x, y;
    } Position;

    Position valid_positions[1000]; // à ajuster selon ton max possible
    int count = 0;

    printf("Voici les cases ou tu peux placer un singe (adjacentes au chemin) :\n");
    ...
    int valide = 0;
    do {
        printf("Ou veux-tu placer ton singe ? (ligne puis colonne) :\n");
        ...
        for (int i = 0; i < count; i++) {
            if (valid_positions[i].x == x_choisie && valid_positions[i].y == y_choisie) {
                ok = 1;
                *x = x_choisie;
                *y = y_choisie;
                break;
        ...
        if (ok) {
            ...
            printf("Cette position n'est pas valide. Choisis une parmi celles proposes.\n");
        }
