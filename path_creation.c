...
Case* generate_path(Case** game_board, int* start_x, int* start_y, int* taille_chemin, int zone_top, int zone_bottom, int zone_left, int zone_right, int height, int width){
      ...
      if(!game_board || !start_x || !start_y || !taille_chemin) return chemin;
      ...
      *taille_chemin = 0;
      ...
      if (case_chemin.pos_y == *start_y) {
            direction = (case_chemin.pos_x < *start_x) ? DROITE : GAUCHE;
      } else {
            direction = HAUT;
      }
      ...
      while (1) {
            ...
            Case* temp = realloc(chemin, (*taille_chemin) * sizeof(Case));
            if (!temp) {
            ...
            direction = choix_direction(derniere_direction, case_chemin,
